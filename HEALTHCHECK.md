...
3. If test 2 fails, dump the system prompt:
   ...
   - Commit and push
   - Update workstation: ssh user@your-server "cd ~/GIT/glueclaw && git fetch origin && git reset --hard origin/main && openclaw gateway restart"
...
0 9 * * * GLUECLAW_KEY=local openclaw agent --agent main --message "say pineapple" 2>&1 | grep -q "pineapple" || echo "GlueClaw blocked - check DETECTION_PATTERNS.md" | mail -s "GlueClaw alert" you@example.com
```
